...
toc
 ...
 finaldata = zeros(length(discovery),length(preparation),length(numBS),length(connectivity),length(densityBL));
 blockageDurations = cell(length(numBS),length(connectivity),length(densityBL));

for indBS = 1:length(numBS)
    nT = numBS(indBS);
